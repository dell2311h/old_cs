class User < ActiveRecord::Base
  ...
  scope :with_followed_by, lambda { |user| select("*").select("(#{Relationship.select('COUNT(follower_id)').where('relationships.followed_id = users.id AND relationships.follower_id = ?', user.id).to_sql}) AS followed") }
